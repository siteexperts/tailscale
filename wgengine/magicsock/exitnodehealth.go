...
//
// The hot path is designed to be cheap: success calls short-circuit on an
// atomic bool load when no warning is set, and both paths compare the
// exit-node key via an atomic pointer load without acquiring c.mu.
